...
vector<FiguraGeometrica *> Interpreter::analise(string filename){
    ifstream Deadpool; // DeDeadpoole o arquivo a ser aberto
    ...
    Deadpool.open(filename.c_str()); // abrir o arquivo

    if (!Deadpool.is_open()){ // verifica se houve errro na abertura do arquivo, se n o estiver aberto ele notifica e encerra o processo
    ...
    while(Deadpool.good()){ // o la o se mantem enquanto o arquvo nao apresenta erro
        getline(Deadpool, s);// pegar a linha do arquivo e associar a s
        if(Deadpool.good()){ // caso o arquivo nao apresente erro
    ...
    Deadpool.close(); // fecha o arquivo
    return figs;
}
